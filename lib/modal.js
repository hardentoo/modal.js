class Modal {
    ...
    for (let [name, commands] of Object.entries(modes)) {
      ...
      for (let [key, object] of Object.entries(commands)) {
        ...
        if (object.events == undefined) {
          ...
          let names = ['up']
          for (let [name, lambda] of Object.entries(object.events)) {
            if (!name in names) {
              throw `${name} event does not exist`
            }
            if (typeof lambda != 'function') {
              throw `${name} should be function`
            }
          }
        }
      }
  ...
  getCommandFromEvent(event) {
    ...
    let object = this.commands[mode][key]
    let defaults = this.commands[mode].default
    return {
      mode: mode,
      key: key,
      default: defaults,
